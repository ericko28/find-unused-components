...
export function scanUsage(projectRoot, components) {
    const sourceFiles = glob.sync('**/*.{vue,js,ts}', {
        cwd: projectRoot,
        absolute: true,
        ignore: ['node_modules/**', '.nuxt/**', 'dist/**', '.output/**', '**/__tests__/**']
    });
    ...
    for (const file of sourceFiles) {
        ...
        while ((match = importRegex.exec(content))) {
            ...
            if (importPath.startsWith('@/components/') || importPath.startsWith('~/components/')) {
                resolvedPath = path.join(projectRoot, 'components', importPath.replace(/^[@~]\/components\//, ''));
            ...
            const comp = components.find(c =>
                normalizePath(c.absolutePath).replace(/\.(vue|js|ts)$/, '') === resolvedPath
            );
            if (!comp) continue;
        ...
        const asyncImportRegex = /defineAsyncComponent\(\s*\(\)\s*=>\s*import\(['"]([^'"]+)['"]\)\s*\)/g;
        let asyncMatch;
        while ((asyncMatch = asyncImportRegex.exec(content))) {
            const asyncImportPath = asyncMatch[1];
            let resolvedPath = '';

            if (asyncImportPath.startsWith('@/components/') || asyncImportPath.startsWith('~/components/')) {
                resolvedPath = path.join(projectRoot, 'components', asyncImportPath.replace(/^[@~]\/components\//, ''));
